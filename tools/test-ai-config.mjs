...
try {
  ...
  for (const [name, body] of Object.entries(bodies)) {
    ...
    await build({ entryPoints: [`app/api/${name}/route.ts`], bundle: true, platform: 'node', format: 'esm', outfile });
    const { POST } = await import(pathToFileURL(outfile).href);
    for (const key of ['说明文字=fake-test-key', 'fake key', 'fake\nkey', '']) {
      process.env.OPENAI_API_KEY = key;
      const response = await POST(new Request(`http://localhost/api/${name}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
      }));
      assert.equal(response.status, 503);
      const text = await response.text();
      if (key) assert.ok(!text.includes(key), 'Response must not echo credential');
    }
  }
  assert.equal(calls, 0);
  console.log('All three AI routes reject malformed credentials before fetch; no credentials echoed.');
} finally {
