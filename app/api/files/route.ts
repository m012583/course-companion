...
export async function DELETE(request: Request) {
  ...
  // Files may be referenced by notes, recycle entries or recovery snapshots.
  // Physical deletion is disabled until reference-aware garbage collection exists.
  return Response.json(
    { error: '为保护笔记引用、回收站和恢复快照，暂不支持物理删除原文件' },
    { status: 409 },
  );
}
