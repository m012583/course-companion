...
**页面“设置与备份”选择的模型会优先于 `OPENAI_MODEL`**；当前界面为固定模型列表，服务必须支持选中的模型。仅修改环境变量不能覆盖页面已选择的模型。固定列表位于 `app/page.tsx` 的 `MODELS` 常量，可按服务实际情况调整。AI 调用费用由所配置的服务账户承担。
...
完整 `npm run lint` 仍有导入版本中通用 UI 组件的已知问题，详见 [测试记录](docs/TESTING.md)。`npm run build` 为单独的构建路径，评审默认采用上面的本地启动流程。

浏览器回归使用独立上下文及内存接口，避免修改真实知识库，详见测试说明。演示素材仅使用测试数据。
